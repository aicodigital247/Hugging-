<?php
/**
 * TradeNexa.com - Authentication - Registration Panel
 * Mobile optimized registration page with automatic simulated coin allocations
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validate_csrf_token($csrf)) {
        $error = "Security token mismatch. Retry.";
    } else {
        $result = auth_register($email, $password, $confirm);
        if ($result['success']) {
...
<head>
    ...
    <title>Register — TradeNexa</title>
...
<body class="flex flex-col min-h-screen justify-center items-center px-4 bg-slate-950">
    ...
    <div class="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-xs text-slate-400 mt-1">Sign Up & Claim $500 Welcome Balance</p>
        </div>
        ...
        <form action="" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" placeholder="you@domain.com" required class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none transition">
            </div>

            <div>
                <label class="block text-slate-400 text-xs font-semibold mb-1 uppercase tracking-wider">Access Password (min 6 chars)</label>
            ...
            <div>
                <label class="block text-slate-400 text-xs font-semibold mb-1 uppercase tracking-wider">Confirm Password</label>
                <input type="password" name="confirm_password" placeholder="••••••••" required class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none transition">
            </div>

            <div class="pt-2">
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-indigo-600/20 cursor-pointer hover:scale-[1.01] active:scale-[0.99] transition">
                    Register & Open Free Account
                </button>
            </div>
        </form>

        <div class="border-t border-slate-800/80 mt-6 pt-4 text-center">
            <span class="text-xs text-slate-400">Already registered on TradeNexa?</span>
            <a href="index.php?page=auth/login" class="text-xs font-bold text-emerald-400 hover:text-emerald-300 ml-1">
                Access Account
            </a>
        </div>
    ...
    <div class="text-[10px] text-zinc-600 mt-4 leading-relaxed text-center uppercase tracking-widest font-mono">
        TradeNexa.com security registry
    </div>
